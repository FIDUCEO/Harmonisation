...
def seriesODR(Hdata, b0, fb):
    ...
    sensors = Hdata[:,12:14].transpose()
    
    def fcnH(coef, Xdata, sens=sensors):
        ...
        s1 = sens[0,:] # 1st sensor index in sensors list (&coeff arr)
        s2 = sens[1,:] # 2nd sensor's index 
...
Im,Hd,Hr,mutime = rhd.rHData(datadir, filelist)
