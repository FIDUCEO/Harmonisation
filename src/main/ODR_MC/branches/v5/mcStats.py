...
def mcStats(mcb0, beta0, sb0):
    ...
    for i in range(3):
        ...
        title = 'Histogram of a[' +str(i)+ '] coeff from MC runs, Gaussian w ' +r'$\mu, \sigma$' +' from ODR eval'
        plt.title(title)
...
def LUdiff(inL, odrLU, mcLU, ttl):
    ...
    LUbias = odrLU - mcLU # bias: ODR eval minus MC eval
