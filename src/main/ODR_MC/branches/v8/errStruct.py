...
def genErr(Hr, Lsys, Tsys, uCs, uCict, slTidx, clen, muCnt):
    ...
    Cs_err, Cs_raw = cer.calc_CC_err(uCs, slTidx, clen)
    err[:,1] = np.repeat(Cs_err, muCnt)  # averaged Space count error per matchup
    
    # Sam's function calc_CC_err to generate Cict averaged errors per scanline
    Cict_err, Cict_raw = cer.calc_CC_err(uCict, slTidx, clen)
    err[:,2] = np.repeat(Cict_err, muCnt) # averaged ICT count error per matchup
...
def genPCS(Hr,Lsys,Tsys,Ccr,Cictr,maWgt,clen,scanlines,blocks,mcounts):
    ...
    errCs = np.random.normal(scale=Ccr) # Cs count error per scanline
    ...
    errCict = np.random.normal(scale=Cictr) # generate Cict error per scanline
