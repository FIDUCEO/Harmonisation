...
from numpy import mean as npmean, ones, zeros
...
def seriesODR(Hdata, Hunc, b0, sensors, fb):
    ...
    X = Hdata[:,0:11].transpose() # transpose data matrix
    Y = Hdata[:,11] # adjustment values K
    bsens = sensors.transpose()
    
    VX = (Hunc[:,0:11]**2).transpose() # squared random uncertainty X vars
    ...
    fit = odr.odr(fcnH,b0,Y,X,ifixb=fb,iprint=3,rptfile='shFinal.rpt',maxit=20)
    mFit = odr.Output(fit)
