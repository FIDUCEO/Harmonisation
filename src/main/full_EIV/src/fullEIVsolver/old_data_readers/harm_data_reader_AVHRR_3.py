...
from nplcore import Uncertainty
...
class MatchUp(MatchUp_template):
    ...
    def openMatchUpData(self, paths, open_uncertainty=True, open_time=True, open_additional_values=None):
        """
        ...
        for i, dir in enumerate(paths):
            ...
            Darray[istart:iend, :] = rootgrp.variables['H'][:, sel_cov]
            uRarray[istart:iend, :] = rootgrp.variables['Ur'][:, sel_cov]
            uSarray[istart:iend, :] = rootgrp.variables['Us'][:, sel_cov]
        ...
        for i, (cov, mu) in enumerate(zip(idx['n_cov'], idx['n_mu'])):
            ...
            if idx['sensors'][idx['n_sensor'][i]] == -1:
                unc[i] = Uncertainty(1, uRarray[istartm:iendm, col_idx])
            ...
            # > if C_Space/C_ICT - averaging correlation (location hard coded)
                ...
                unc[i] = Uncertainty(3, (len(w_matrices)-1, len(uncertainty_vectors)-1))
            ...
            # > systematic effect
                ...
                unc[i] = Uncertainty(2, (uRarray[istartm:iendm, col_idx],
        ...
        for i in range(len(idx['Nm'])):
            ...
            unck[i] = Uncertainty(1, uKarray[istart:iend])
        ...
        return values, unc, w_matrices, uncertainty_vectors, ks, unck, time1, time2, None, idx
