...
bsub -W 48:00 -x -R "select[maxmem > 128000] rusage[mem=250000]" -q par-single  -M 400000000 -o ${logPathPC} < ${scriptPathPC} &>/dev/null
...
bsub -W 48:00 -x -R "select[maxmem > 128000] rusage[mem=250000]" -M 400000000 -q par-single -o ${logPathT} < ${scriptPathT} &>/dev/null
...
while [ $stop -eq 0 ]; do
	...
	then
        ...
        bsub -W 48:00 -x -R "select[maxmem > 128000] rusage[mem=250000]" -M 400000000 -q par-single -o ${logPath} < ${scriptPath} &>/dev/null
