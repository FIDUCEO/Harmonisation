...
class GNAlgo:
    ...
    def run(self, tol=1e-6, tolA=1e-8, tolB=1e8, tolU=1e-8, show=False, return_covariance=True):
        ...
        values_res = self.unconvert_values(f[:N_var], self.HData.unc, self.HData.idx, self.HData._original_idx)
    ...
    def unconvert_Xs(self, xyza, unc, idx, n_sensor, n_mu):
        ...
        for n_cov in xrange(1, N_cov + 1):
            ...
            if block_unc.typeID == 3:
                w_matrix = self.HData.w_matrices[block_unc.w_i]
                u_matrix = self.HData.u_matrices[block_unc.u_i]

                Xs.append(w_matrix.dot(u_matrix*xyza[ib:ie]))
                        ...
                        if block_unc.typeID == 3:
                            ...
                            if not transpose:
                                JPx[istart:iend] = JPx[istart:iend] \
                                                        + s*JB*JR[:, n_cov-1]*w_matrix.dot(u_matrix * x[ib:ie])/uK
                            elif transpose:
                                JPx[ib:ie] = JPx[ib:ie] \
                                                        + s*u_matrix*w_matrix.T.dot(JB*JR[:, n_cov-1]*x[istart:iend]/uK)
    ...
    def unconvert_values(self, values_con, unc, idx, idx_orig):
        ...
        for i in xrange(len(idx['n_mu'])):
            ...
            if block_unc.typeID == 3:
                ...
                values = append(values, w_matrix.dot(u_matrix * values_con[ib:ie]))
    ...
    def save(self, directory):
        ...
        values_t_initial_var = dataset.createVariable('values_t_initial', 'f8', ('M_T',), zlib=True, complevel=9)
        ...
        variables_t_estimate_var = dataset.createVariable('variables_t_estimate', 'f8', ('V_T',), zlib=True,
                                                          complevel=9)
        ...
        # > values_t_initial variable
        preconditioner_solution_var = dataset.createVariable('preconditioner_solution', 'f8', ('n', 'n',), zlib=True,
                                                             complevel=9)
